...
export default {
  ...
  methods: {
    ...
    updateBirthday() {
      ...
      // this.$emit('updateBirthday',data)
      updateBirthday(data).then((response) => {
        this.msgSuccess('操作成功')
        this.$emit('updateBirthday',this.customer.externalUserid)
        this.datePickerVisible = false
      })
    },
    // getDetail(externalUserid){
    //   getDetail(externalUserid).then(({ data }) => {
...
<template>
<el-dialog :title="title" :visible.sync="CustomerDetail" >
    ...
    <el-card shadow="never" :body-style="{ width: '400px' }">
      <div>
        <el-button
          v-if="customer.weFlowerCustomerRels&&customer.weFlowerCustomerRels[0].status == 1"
          class="fr"
          type="danger"
          plain
          size="mini"
