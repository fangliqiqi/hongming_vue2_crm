<template>
  <div class="app-container">
    ...
    <!-- 客户列表渲染 -->
    <div class="tabbar-vi">
      <el-tabs v-model="activeType" @tab-click="handleQuerys">
        <!-- <el-tab-pane label="回收站"  name="20" value='20'></el-tab-pane> -->
        <el-tab-pane label="全部客户"  name="20"></el-tab-pane>
        <el-tab-pane label="全部订单"  name="30"></el-tab-pane>
      </el-tabs>
    ...
    <div class="table-data">
      ...
      >
        ...
        <el-table-column label="客户名称" align="left" prop="companyName"/>
        <!-- <el-table-column label="删除人" align="left" prop="recyclePeople"/> -->
        <el-table-column
          label="删除人"
          align="left"
          prop="recyclePeople"
        >
          <template slot-scope="scope">
            {{scope.row.recyclePeople}}
          </template>
        </el-table-column>
        <el-table-column label="删除时间" align="left" prop="recycleTime" />
        <el-table-column
          align="left"
          class-name="table-control"
        >
          ...
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-hasPermi="['crm:userlabel:edit']"
              class="red"
              @click="clientThoroughDelet(scope.row)"
            >彻底删除</el-button
            >
          </template>
        </el-table-column>
...
import {
  sourceClient,
  clientRestore,
  clientRecycleBinDelete,
  RecycleBinOrderListPage,
  restoreOrder,
  deleteOrderRecycleBin
} from "@/api/crm/RecycleBin/RecycleBin";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      }else if(this.activeType=='30'){
        console.log('全部订单')
        data.churnType = null
        RecycleBinOrderListPage(data).then(response=>{
          this.recycleBinOrderList = response.data.rows;
          this.total = response.data.totalRows;
          this.loading = false;
        })
      }
    ...
    handleRestore(row) {
      ...
      let type = this.activeType;
        if (row.id) {
          this.$confirm('是否还原该'+(type==20?'订单':'客户')+'?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'

          }).then(function() {
            return restoreOrder({ id: id})
          }).then(() => {
            this.getList()
    ...
    // 彻底删除
    clientThoroughDelet(row) {
      let type = this.activeType;
       if (row.id) {
         this.$confirm('是否确认删除该'+(type==20?'订单':'客户')+'?', '提示', {
           confirmButtonText: '确定',
           cancelButtonText: '取消',
           type: 'warning',

         }).then(function() {
            return deleteOrderRecycleBin({ id: row.id})
