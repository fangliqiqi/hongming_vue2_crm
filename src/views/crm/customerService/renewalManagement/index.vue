<template>
  <div class="app-container renewalManagement">
    <!-- 首行搜索框 -->
    <div class="search-vi">
      <el-row :gutter="24">
        <el-col :span="24">
    ...
    <div class="tabbar-vi">
      <el-tabs v-model="queryParams.kfAuditType" @tab-click="handleQuerys">
        ...
        <el-tab-pane label="合同过期客户" value="500" name="500"> </el-tab-pane>
      ...
      <div class="account-table table-data">
        ...
        >
          ...
          <el-table-column label="合同签约时间" align="left" width="180">
            <template slot-scope="scope">
              {{
                parseTime(scope.row.signingDate, "{y}-{m}-{d} {h}:{i}:{s}") ||
                ""
              }}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      if (data.kfAuditType != "500") {
        this.queryParams.timeScreening = data.kfAuditType;
        ExpiringCustomers(this.queryParams).then((response) => {
          this.arrearsList = response.data.records || [];
          this.total = response.data.total;
          this.loading = false;
        });
      } else if (data.kfAuditType == "500") {
