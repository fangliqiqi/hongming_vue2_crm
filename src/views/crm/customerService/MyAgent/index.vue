<template>
  <div class="app-container customer_MyAgent">
    <!-- 首行搜索框 -->
    <div class="search-vi">
      <el-row :gutter="24">
        <el-col :span="24">
    ...
    <!-- 状态切换 -->
    <div class="tabbar-vi">
      <el-tabs v-model="queryParams.flowStatus" @tab-click="handleQuerys">
    ...
    <div class="account-table table-data">
      ...
      >
        ...
        <!-- <el-table-column label="服务项目" prop="serpriceName" width="150" /> -->
        ...
        <el-table-column label="合同起始日期" width="180">
          <template
            slot-scope="scope"
        ...
        >
          <template slot-scope="scope">
            ...
            >
              <el-col
                :span="6"
                :class="scope.row.createBy ? ' bgorange' : 'bggrey'"
              >
                <div>业</div>
              </el-col>
              ...
              <el-col
                :span="6"
                :class="scope.row.customerServiceName ? ' bgorange' : 'bggrey'"
              >
                <div>客</div>
              </el-col>
              ...
              <el-col
                :span="6"
                :class="scope.row.foreignAccountName ? ' bgorange' : 'bggrey'"
              >
                <div>账</div>
              </el-col>
              <div style="margin-right: 2px">
            ...
            <el-row>
              ...
              <el-col :span="12" v-if="scope.row.flowStatus != 10">
                <el-button
                  ...
                  size="mini"
                  v-hasPermi="['share:bank:edit']"
                  @click="handleSign(scope.row.companyId)"
                >
                  标记</el-button
                >
              </el-col>
              ...
              <el-col :span="12" v-if="queryParams.flowStatus == 10">
                <el-button
                  type="success"
                ...
                <div>
                  ...
                  >
                    ...
                    <el-table-column
                      ...
                      prop="serpriceName"
                    />
                    <el-table-column label="已收情况" width="80px">
                      <template slot-scope="scope">
                        <!-- <span
                    ...
                    <el-table-column label="已收金额" prop="hasMoney">
                      <!-- scope.row.auditStatus == 10?scope.row.serviceMoney:scope.row.actualPaymentAmount-->
                      <template slot-scope="scope">
                        {{ scope.row.hasMoney }}
                      </template>
                    </el-table-column>
                    <el-table-column label="未收金额" prop="unhasMoney">
                      <!-- scope.row.auditStatus == 10?scope.row.actualPaymentAmount:scope.row.amountInArear -->
                      <template slot-scope="scope">
                        {{ scope.row.unhasMoney }}
                      </template>
                    </el-table-column>
                      ...
                      <el-col :span="12">
                        ...
                        >
                          ...
                          oninput="value=value.replace(/[^\d^\.]/g,'')"
                            v-model="
                              templateSelection.currentRepaymentAmount
                            "
                            type="text"
                            placeholder="请输入预收金额"
                          ></el-input> -->
                          <el-input-number
                            style="width: 100%;"
                            v-model="templateSelection.currentRepaymentAmount"
                            placeholder="请输入预收金额"
                            :controls='false'
                            :precision='2'
                            :min="0" >
                  ...
                  <div class="contract_list">
                    <div class="child">
                      <div class="title">
                        <div class="link"></div>
                        <div class="font">
                          {{ "合同编号:" + contract.controlNo }}
                        </div>
                        <el-button
                          type="warning"
                          @click.native="renew(contract)"
                          size="mini"
                          plain
                          v-if="
                            (contract.marketBusirSerprices || []).some(
                              (item) => item.accountType == 20
                            )
                          "
                          :disabled="contract.renewType == 10"
                          >续费</el-button
                        >
                      </div>
                      <el-row class="kind">
                          ...
                          <div class="name">
                            ...
                            <span
                              >签约日期:<span class="span">{{
                                parseTime(contract.signingDate, "{y}-{m}-{d}")
                              }}</span></span
                            >
                            <span
                              >合同有效期:<span class="span"
                                >{{
                                  parseTime(contract.startTime, "{y}-{m}-{d}")
                                }}至{{
                                  parseTime(contract.endTime, "{y}-{m}-{d}")
                                }}
                              </span>
                            </span>
                          </div>
                          ...
                          <div class="name">
                            ...
                            <span
                              >收费周期:<span class="span">{{
                                contract.cycleType == 10
                                  ? "按月收费"
                                  : contract.cycleType == 20
                                  ? "按季度收费"
                                  : contract.cycleType == 30
                                  ? "按半年收费"
                                  : contract.cycleType == 40
                                  ? "按年收费"
                                  : ""
                              }}</span></span
                            >
                        ...
                        <el-col :span="4">
                          <div class="name">
                            ...
                            <span
                              v-for="val in contract.marketCompbusiImgfileResults"
                              :key="'key' + val.id"
                            >
                              <el-image
                                fit="cover"
                                :src="fileUrl + val.url"
                                :preview-src-list="
                                  contract.marketCompbusiImgfileResults.map(
                                    (ite) => {
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <div style="font-size: 16px">
              ...
              />客户名称：{{ auditDetailsForm.companyName }}
            </div>
          </el-col>
        </el-row>
        <el-row>
    ...
    >
      ...
      >
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="dialogCancel">取消</el-button>
          <el-button type="primary" @click="addNowFollow">立即添加</el-button>
        </div>
      </el-dialog>
      <el-row :gutter="24">
        <el-col :span="12">
    ...
    >
      ...
      <!-- <el-form ref="distributionForm" :model="distributionForm" :rules="rules">
        <el-row :gutter="14">
          ...
          <el-col :span="12">
            <el-form-item label="部门人员" prop="userName">
              <el-select
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // queryParams.kfAuditType =
      //   queryParams.kfAuditType == 0 ? null : queryParams.kfAuditType;
      // if (queryParams.kfAuditType == 40) {
      //   queryParams.unassigned = 10;
      //   queryParams.kfAuditType = null;
      // } else {
      //   queryParams.kfAuditType =
      //     queryParams.kfAuditType == 0 ? null : queryParams.kfAuditType;
      // }
      queryParams.flowStatus == 0
        ? (queryParams.flowStatus = null)
        : (queryParams.flowStatus = queryParams.flowStatus);
      MySpecialApproval(queryParams).then((response) => {
    ...
    handleDispatch(row) {
      ...
      if (row.service == "") {
        ...
        this.$confirm("是否重新分配?", "警告", {
          ...
          .then(() => {
            this.dispatchDialog = true;
            return entDept(this.queryParams);
          })
          .then((response) => {
            var tep = response.data;
            for (var a in tep) {
    ...
    handleDepartment(row) {
      this.distributionForm.userId = row.userId;
      this.distributionForm.userName = row.name;
      this.distributionForm.role = row.role;
      this.distributionForm.roleCode = row.roleCode;
    },
    handleDepartments(row) {
      this.distributionForm.extUserId = row.userId;
      this.distributionForm.extUserName = row.name;
      this.distributionForm.extRole = row.extRole;
      this.distributionForm.extRoleCode = row.roleCode;
    },
    ...
    handlefindReturned(row) {
      ...
      findReturnedMoney({ id: row }).then((response) => {
        response.data.returnedMoney.map((items) => {
          items.hasMoney =
            items.auditStatus == 10
              ? items.serviceMoney
              : items.actualPaymentAmount;
          items.unhasMoney =
            items.auditStatus == 10
              ? items.actualPaymentAmount
              : items.amountInArear;
    ...
    //选择项目收费
    getCurrentRow(row) {
      this.templateSelection.id = row.id;
    ...
    confirmCharge() {
      this.$refs["templateSelection"].validate((valid) => {
        if (valid) {
          (this.templateSelection.actualPaymentAmount = 0),
            (this.templateSelection.serviceMoney =
              this.templateSelection.currentRepaymentAmount),
            (this.templateSelection.amountInArear =
              this.templateSelection.currentRepaymentAmount),
            MarketBusirSerpriceCharge(this.templateSelection).then(
              (response) => {
                this.$nextTick(() => {
                  this.$refs.uploadCharge.clearFiles();
                  this.$refs["templateSelection"].resetFields();
    ...
    paymentCollection() {
      ...
      this.paymentForm.id = this.templateSelection.busirSerpriceId;
    ...
    renew(row) {
      if (row.renewType == "30") {
        ...
        console.log("hahahah");
        findPayMethod().then((res) => {
          console.log(res.data[0].cityCode);
          this.renewForm.payCode = JSON.parse(res.data[0].cityCode);
        });
        this.renewals = true;
        this.renewForm = row;
        this.renewForm.contractFileIds = "";
    ...
    followUp(row) {
      ...
      FollowRecord({ id: this.followForm.id, classify: 30 }).then(
        (response) => {
    ...
    resetForm() {
      Object.assign(
        this.$data.templateSelection,
        this.$options.data().templateSelection
      );
    },
    // 表单重置
    reset() {
    ...
    resetQuery() {
      ...
      this.followDate = "";
      this.writeTime = "";
      // this.queryParams.outerAccounting = null;
      // this.queryParams.service = null;
      // this.queryParams.followDate = null;
      // this.queryParams.writeTime = null;
      // this.queryParams.labelName = "";
      // this.handleQuery();
    },
    //收费登记多选框选中数据
    ...
    payment(val) {
      ...
      this.houproPaymentSettingList.forEach((item) => {
        if (this.templateSelection.paymentMethod == item.patternPayment) {
          ...
          // console.log(this.templateSelection.bankType);
        }
      });
      this.CollectioAccountList = [];
      this.bankOfDepositList = [];
