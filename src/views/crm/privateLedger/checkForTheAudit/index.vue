<template>
  <!-- checkForTheAudit -->
  <div class="app-container checkForTheAudit">
    <div class="search-vi">
      <el-row :gutter="24">
        <el-col :span="24">
          <el-popover
            ...
            <div class="search-popover">
              ...
              >
                ...
                <el-form-item label="审核状态:">
                  <template>
                    <el-radio
                      v-model="queryParams.advanceReviewerType"
                      label="10"
                      >待审核</el-radio
                    >
                    <el-radio
                ...
                <el-form-item label="收款人:">
                  <el-input
                    placeholder="请输入收款人"
                    v-model="queryParams.advanceTollReviewer"
                  ></el-input>
                </el-form-item>
                <el-form-item label="审核时间:">
                  <el-date-picker
                    v-model="queryParams.advanceReviewerTime"
                    size="small"
                    class="mw350"
    ...
    <div class="table-data">
      ...
      >
        ...
        <el-table-column label="公司名称" width="155"  prop="companyName">
          <template slot-scope="scope">
            <div class="teFlex">
              <el-tooltip :content="scope.row.applyContent" placement="top">
                <span v-if="scope.row.specialWhether == '10'" class="teIcon"
                  >特</span
                >
              </el-tooltip>
        ...
        />
        <el-table-column label="未收金额" align="left" prop="paymentAmount" />
    ...
    <el-dialog
      title="付款截图"
      :visible.sync="paymentScreenshotCondition"
      width="516px"
      append-to-body
    ...
    >
      <el-form label-position="labelPosition" class="chengjiao">
        <el-row>
          <el-col
            :span="12"
            v-for="(item, index) in paymentScreenshotList"
            :key="index + 'm'"
          >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="paymentScreenshotCondition = false"
          >关闭</el-button
        >
        ...
        <el-row :gutter="24" class="rowFather">
          ...
          <el-col :span="12">
            <el-form-item label="收款方式">
              <el-select
                v-model="auditData.paymentMethod"
                placeholder="请选择"
                :disabled="true"
              >
                <el-option
          ...
          <el-col :span="24">
            <el-form-item label="收费备注">
              <el-input
                ...
                autosize
                placeholder="请输入内容"
                v-model="auditData.chargeRemarks"
                :disabled="true"
              >
              </el-input>
            </el-form-item>
          ...
          <el-col
            :span="12"
            v-for="(item, index) in collectionScreenshotList"
            :key="index + 'm'"
          >
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item
              ...
              prop="dispatchState"
            >
              <el-radio-group v-model="auditData.dispatchState">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="orderParticulars = false"
          >关闭</el-button
        >
      </div>
    </el-dialog>
...
import { getClient } from "@/api/crm/customer/customer";
...
import { listOrder, getServeList } from "@/api/crm/order";
import render from "@/utils/generator/render";

export default {
  name: "checkForTheAudit",
  components: {
    EditDrawer,
    EditOrder,
    NewOrder,
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        paymentTimes: null, //付款时间
        paymentStartTime: null, //   付款开始时间
        paymentEndTime: null, //  付款结束时间
  ...
  methods: {
    ...
    paymentScreenshot(row) {
      ...
      findChargeList({ id: row.id, identification: row.identification }).then(
        (res) => {
    ...
    getList() {
      ...
      this.queryParams.advanceReviewerType =
        this.queryParams.advanceReviewerType == "0"
          ? "0"
          : this.queryParams.advanceReviewerType;

      if (data.paymentTimes) {
        // console.log('付款' + data.paymentTimes)
        data.paymentStartTime = data.paymentTimes[0];
        data.paymentEndTime = data.paymentTimes[1];
        data.paymentTimes = null;
      } else {
        data.paymentStartTime = null;
        data.paymentEndTime = null;
      }
    ...
    getOrderList(id) {
      listOrder({ id: id }).then((res) => {
        ...
        this.stepList = this.orderList.res;
      });
    },
    //点击订单编号
    editOrderFun(row) {
      ...
      console.log(row);
      getServeList({ id: row.busirId }).then((res) => {
        this.stepList = row.res;
        console.log("出来了");
    ...
    orderAuditUp(row) {
      ...
      selectChargeVouchers({
        ...
        this.auditData.internalAccountImages =
          response.data.internalAccountImages || [];
        // console.log(response.data);
        if (
          this.auditData != {} &&
          this.auditData.internalAccountImages.length > 0
        ) {
          this.auditData.internalAccountImages.forEach((ite) => {
            this.collectionScreenshotList.push(ite);
          });
        }
