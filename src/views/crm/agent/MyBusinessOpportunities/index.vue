     ...
     <el-table-column
        ...
        >
          <template slot-scope="scope">
              <div v-if="scope.row.isOtherFirmGeneration==1">是</div>
              <div v-if="scope.row.isOtherFirmGeneration==2">否</div>
          </template>
        </el-table-column>
        <el-table-column
          label="标签"
        ...
        <div class="product-vi" v-show="stepIndex == 1">
          ...
          <div class="product-list">
            <el-row :gutter="20" style="margin-top: -20px">
              <el-col v-for="item in proTypeOption" :key="item.index" :span="8">
                      ...
                      <el-form-item label="备注">
                        ...
                        >
                          <el-input
                            v-model="item.remark"
                            placeholder="请填写备注"
                            @change="updateContactFun({ remark: $event }, item)"
                          ></el-input>
                        </el-tooltip>
                      </el-form-item>
...
import { BusinessOpportunityList } from "@/api/crm/agent/MyBusinessOpportunities";
...
export default {
  ...
  created() {
    ...
    //纳税性质
    this.getDicts("vat_types").then((res) => {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (
        to.path == "/agent/MyBusinessOpportunities" &&
        from.path == "/index" &&
        to.query.add == "true"
      ) {
        vm.handleAdd();
  ...
  methods: {
    ...
    handleDepartment(val) {
      this.wxDepartmentStaff.forEach((item) => {
        ...
        if (item.userId == val) {
          ...
          // this.distributionForm.firmId = this.distributionForm.id;
          this.distributionForm.firmId = item.companyId;
    ...
    getList() {
      ...
      BusinessOpportunityList(data).then((response) => {
