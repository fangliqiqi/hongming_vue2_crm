  ...
  <div class="app-container">
    ...
    <div class="table-data">
      ...
      >
        ...
        <el-table-column label="业务描述" align="left" prop="categoryText" />
        <el-table-column label="标签" align="left" prop="userLabel">
          <template slot-scope="scope">
            <el-tag v-for="(tag, index) in scope.row.userLabel" :key="index">
              {{ tag }}</el-tag
            >
          </template>
        </el-table-column>
        <el-table-column label="意向等级" align="left" prop="sourceTypeLv" />
        <el-table-column label="商机来源" align="left" prop="customerSource"  :formatter="delSwitchFormat" />
        <el-table-column
          label="最近分配时间"
          align="left"
          prop="accomplishTime"
        />
        <el-table-column label="最近联系时间 " align="left" prop="" />
        <el-table-column
          label="操作"
          align="left"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      BusinessOpportunity(data).then(response => {
        this.BusinessList = response.data.rows;
        this.total = response.data.totalRows;  
           this.BusinessList.forEach(item => {
          item.userLabel = JSON.parse(item.userLabel);
        });
      }).finally(()=>{
this.loading = false;
      });
    ...
    handleRob(row) {
      let id = row.id;
      vieForCommercial({ id: id }).then(response => {
        this.getList();
        this.msgSuccess("抢单成功");
      });
    },
    //分配
