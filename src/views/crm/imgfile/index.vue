     ...
     <el-col :span="12">
        ...
        <el-form-item label="附件名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入附件名称" />
           ...
           </el-row>  <el-row :gutter="20"> <el-col :span="12">
...
<script>
import { listImgfile, getImgfile, delImgfile, addImgfile, updateImgfile, exportImgfile } from "@/api/crm/imgfile";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        state: [
          { required: true, message: "图片文件属类型不能为空", trigger: "blur" }
        ],
        type: [
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加个人业务订单附件";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getImgfile({id:row.id}).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改个人业务订单附件";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      const ids = row.id 
      this.$confirm('是否确认删除个人业务订单附件编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delImgfile({id:row.id});
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有个人业务订单附件数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
