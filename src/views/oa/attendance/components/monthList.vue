...
export default {
  ...
  methods: {
    ...
    exceptionTypeFormat(row) {
      return this.selectDictLabel(this.checkRecordTtype, row.exception);
    },
    // // 字典翻译 考勤假勤类型
    leaveTtypeFormat(row) {
      return this.selectDictLabel(this.checkRecordTtype, row.type);
    },


    // 根据部门获取人员 同时头部加上全部人员的选择项
    getUsers(data){
     ...
     getList(){
       if(!this.queryForm.useridlist){
         ...
         this.$message({ showClose: true, message: '请选择考勤时间', type: "error" });
          ...
          let date = new Date(this.queryForm.starttime)
          var yaer = date.getFullYear()
          var month = date.getMonth() + 1
          var late = new Date(yaer, month, 0).getDate();
          this.queryForm.endtime = this.queryForm.starttime.replace(/(.*)01/,'$1'+late)
          this.loading = true
         api.getMonthList(this.queryForm).then(res=>{
  ...
  <div class="page app-container">
   ...
   <el-table
    ...
    >
      ...
      <el-table-column prop="summary_info.work_days" label="应打卡天数" align="center"></el-table-column>
      <el-table-column prop="summary_info.regular_days" label="正常天数" align="center"></el-table-column>
      <el-table-column prop="summary_info.except_days" label="异常天数" align="center"></el-table-column>
      <el-table-column prop="summary_info.regular_work_sec" label="实际工作时长(h)" align="center">
