<template>
  <div class="HouseBillOfLading">
    ...
    >
        ...
        <el-form v-model="form" label-position="top" ref="houseForm">
          ...
          <el-row :gutter="22" v-if="!(form.signbillType == 30)">
           ...
           <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="收款账号" prop="patternPayment">
                <el-select
                  v-model="form.CollectioAccount"
                  placeholder="请选择收款账号"
                  :disabled="true"
                >
                  <el-option
          ...
          <el-row :gutter="22" v-if="tidan == 'yes'">
            <el-col :span="24">
      ...
      <div class="btn">
        <el-button v-if="(form.endingParagraph == 0||tidan=='modify'||tidan=='oderModify') && userRole.indexOf('salesmanEstate') !== -1" @click="Submit(20)">
          暂存</el-button
        >
        <el-button
          type="primary"
          @click="Submit(10)"
          v-if="(form.endingParagraph == 0||tidan=='modify'||tidan=='oderModify') && userRole.indexOf('salesmanEstate') !== -1"
          >提交</el-button
        >
        ...
        <el-button
          type="warning"
          @click="handleSpecial"
          v-if="form.endingParagraph > 0 && this.tidan == 'no'"
        >
          特殊审批</el-button
        >
        <el-button
        ...
        <el-button type="danger" @click="reject(30)" v-if="this.tidan == 'yes'">
          驳回</el-button
        >
        <el-button type="primary" @click="reject(20)" v-if="this.tidan == 'yes'"
          >通过</el-button
        >
      </div>
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="付款账户" prop="payAccount">
                <el-input
                  placeholder="请输入付款账户"
                  v-model="balanceForm.payAccount"
                >
                </el-input>
              </el-form-item>
            </el-col>
...
export default {
  ...
  watch: {
    houseBillOfLadingShow(oldVal, newVal) {
      this.showDialog = this.houseBillOfLadingShow;
    }
  },
  created() {
...
this.form.houproOrderCustomerAccountList.splice(i,1);
    ...
    payment() {
      this.houproPaymentSettingList.forEach(item => {
        if (this.form.patternPayment == item.id) {
          this.form.currentRate = item.currentRate;
        }
      });
      // console.log(this.form.currentRate);
    },
    //提单回显
    echo(row) {
      ...
      if (row.tidan == "yes") {
        ...
        findOrderAttache(row).then(response => {
          let data = response.data;
            data.cityCode = data.cityCode.replace(
          "[",
          ""
        );
        data.cityCode = data.cityCode.replace("]", "");
...
bringOrder(this.form).then(response => {
      ...
      }else if (this.form.immediatelyType == "N" && this.form.endingParagraph > 0) {
